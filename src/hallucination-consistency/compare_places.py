...
with open("../../data/hallucination-consistency/us_places.json", "r") as f:
    ...
    for place in places:
        ...
        state = place["state"]
        for dir in all_dirs:
            with open(f"../../logs/hallucination-consistency/places/{dir}/qa_{index}.txt", "r") as f:
                ...
                result = compare("state", place["state"], answer)
                print(f"ground-truth: {place['state']}, answer: {answer}, result: {result}")
                compared_results[index]["answers"].append(answer)
                compared_results[index]["labels"].append(result)
        index += 1
    with open("../../logs/hallucination-consistency/places/compared_results.json", "w") as f:
