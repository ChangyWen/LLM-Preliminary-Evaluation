...
def compare(entity_type, ground_truth, answer):
    messages = [
        {"role": "user", "content": f"Please compare the following answer and the ground truth regarding the {entity_type} name (do not need to be exact match). The answer is '{answer}'. The ground truth is '{ground_truth}'. Response only '1' if the answer mention the ground-truth {entity_type} name, otherwise response only '0' (no explanation needed)."}
    ]
...
with open("../../data/hallucination-consistency/historical_figures.json", "r") as f:
    ...
    for figure in figures:
        ...
        name = figure["name"]
        for dir in all_dirs:
            with open(f"../../logs/hallucination-consistency/historical_figures/{dir}/qa_{index}.txt", "r") as f:
                ...
                result = compare("historical figure", name, answer)
                print(f"ground-truth: {figure['name']}, answer: {answer}, result: {result}")
                compared_results[index]["answers"].append(answer)
                compared_results[index]["labels"].append(result)
        index += 1
    with open("../../logs/hallucination-consistency/historical_figures/compared_results.json", "w") as f:
