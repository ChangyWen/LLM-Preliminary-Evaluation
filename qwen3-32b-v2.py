...
def generate(directory, file_index, prompt, tokenizer, model, enable_thinking, do_sample, temperature):
    ...
    os.makedirs(f"./logs/Qwen3-32B/thinking_{enable_thinking}/{directory}", exist_ok=True)
    with open(f"./logs/Qwen3-32B/thinking_{enable_thinking}/{directory}/qa_{file_index}.txt", "w") as f:
        # write to the file
        f.write("**************************************** Inputs ****************************************\n")
